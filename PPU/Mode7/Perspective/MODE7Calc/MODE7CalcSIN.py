...
while angle < points:
  print ("M7SINTable%d: // Mode7 +SIN (B) Table 224 * Rotation / Scaling Ratios (Last 8-Bits Fractional)" % angle)
  ...
  while REG_VCOUNT <= 224:
    ...
    M7_SIN =  g_sinf * lam # M7B = +SIN(angle) * ScaleXY

    print ("db $01; dw", round(M7_SIN*256), "// Repeat 1 Scanline, Mode7 +SIN Scanline", REG_VCOUNT)
    REG_VCOUNT += 1
